...
| 2026-06-20 | Swagger UI по корневым адресам: добавлены маршруты `/docs/`, `/api-docs/` (Swagger), `/redoc/` (ReDoc) в `config/urls.py` (схема переиспользуется, `/api/docs/` сохранены); nginx regex-location `^/(docs\|redoc\|api-docs)(/\|$)` проксирует их на backend. Преподаватель открывает `http://localhost:8888/docs`. +`test_docs.py` (6 путей → 200), 83 теста. |
| 2026-06-20 | Загрузка фото рецепта. Бэкенд уже принимал `photo` (ImageField, MultiPartParser по умолчанию) — добавлен только UI. Из-за вложенных полей фото грузится отдельным multipart-PATCH `/recipes/{id}/` после JSON-создания/обновления. `recipesApi.uploadPhoto` (FormData, Content-Type undefined → boundary), хук `useUploadRecipePhoto`, в `RecipeFormPage` — input file + превью + валидация (image, ≤5МБ). +2 backend-теста (upload ok / non-image 400), 85 тестов. |

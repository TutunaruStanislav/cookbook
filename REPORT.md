...
## 2026-06-19 — Починен планировщик меню: перенос блюд и отображение названий

### Симптом (по запросу заказчика)
- При переносе уже запланированного блюда (напр. завтрак с понедельника на вторник) — «всё слетает».
- Не видно названий запланированных блюд, неинформативно.

### Диагноз — один корень
Сериализатор слота (`MealSlotSerializer`) отдаёт `recipe` как **числовой PK** (для записи через PATCH), а данные для отображения — во вложенном **`recipe_detail`** (`{id, title, photo, cooking_time, difficulty, author_name}`). Это корректный DRF-паттерн. Но frontend-тип `MealSlot.recipe` ошибочно был объявлен как `RecipeList`, и `PlannerPage` читал `slot.recipe.title` / `slot.recipe.id`. На рантайме `slot.recipe` — число, поэтому:
- `slot.recipe.title` = `undefined` → чип рисовался с **пустым названием** (баг №2).
- В drag-логике `srcSlot.recipe.id` = `undefined` → на целевой слот уходил `{recipe: undefined}`, axios выкидывал ключ → пустой PATCH (целевой слот не менялся), а исходный затирался в `null`. Блюдо **исчезало** (баг №1).

### Что сделано (frontend, бэкенд корректен — не трогался)
- **`types/index.ts`**: добавлен тип `SlotRecipe`; `MealSlot.recipe` → `number | null` (PK), добавлено `recipe_detail: SlotRecipe | null`.
- **`PlannerPage`**: отображение (чип, drag-overlay, проверка занятости) переведено на `recipe_detail`; drag-swap оперирует PK (`slot.recipe`). Логика swap сохранена: `dst ← src.recipe`, `src ← dst.recipe ?? null` (перенос в пустой слот = исходный пустеет; на занятый = обмен).
- В чипе к названию добавлена строка «сложность · время» — стало информативнее.

### Проверка
- Симуляция на live-API (под alice): swap двух занятых слотов `40 (Овсяная каша) ↔ 47 (Котлеты)` — оба блюда на месте, ничего не исчезло; перенос с очисткой (`recipe: null`) работает; данные возвращены к исходным. `recipe_detail.title` приходит заполненным.
- `tsc` — 0 ошибок; **20 frontend-тестов** проходят; фронтенд пересобран, `:8888` отдаёт 200.

### Принятое решение
- Чиним контракт на стороне клиента (выравниваем тип под API), а не меняем сериализатор: `recipe` (PK для записи) + `recipe_detail` (объект для чтения) — здоровый DRF-паттерн, ломать его в угоду фронту не нужно.

---

## 2026-06-19 — Планировщик: до 3 блюд в одном слоте

### Запрос заказчика
Возможность класть в один слот (день × приём пищи) до 3 блюд.

### Уточнения (согласовано с заказчиком)
- Drag-and-drop переносит **одно** блюдо (добавляется в целевой слот, если есть место).
- Перенос/добавление в **полный** слот (3 блюда) — **запрещено** (no-op); кнопка «+» прячется при 3.
- **Дубликаты** в пределах одного слота **запрещены** (повторное добавление игнорируется).

### Изменение модели данных
Раньше слот хранил единственную FK `recipe`. Введён child-модель **`MealSlotRecipe`** (`slot`, `recipe`, `position`), а поле `recipe` у `MealSlot` удалено.
- `unique_together [('slot','recipe'), ('slot','position')]` — нет дублей в слоте + стабильный порядок.
- `MAX_PER_SLOT = 3`.
- Миграция `0002_mealslotrecipe`: создание таблицы → **data-migration** (перенос существующих `slot.recipe` → item, position 0) → удаление поля `recipe`. Обратимая (best-effort reverse).

### Backend
- Новые эндпоинты вместо `PATCH /slots/{id}/ {recipe}`:
  - `POST /menu-plan/slots/{slotId}/items/ {recipe}` — добавить (201; дубль → 200 idempotent; полный слот → 400; чужой приватный → 400).
  - `PATCH /menu-plan/items/{itemId}/ {slot}` — перенести в другой слот (полный/дубль → 400).
  - `DELETE /menu-plan/items/{itemId}/` — убрать.
- `MealSlotSerializer.items` (вложенные `MealSlotItemSerializer` с `recipe` + `recipe_detail`). `photo` отдаётся относительным URL (как у рецептов).
- Список покупок считает вхождения по всем items (рецепт дважды = ×2) — поведение сохранено.
- `seed.py` переведён на `MealSlotRecipe` (+ демо: в Пн-завтраке 3 блюда, в Пн-обеде 2).
- admin: инлайн `MealSlotRecipe` под `MealSlot`.

### Frontend
- Типы: `SlotItem` + `MealSlot.items: SlotItem[]`, `MAX_DISHES_PER_SLOT = 3`.
- `plannerApi`: `addSlotItem` / `moveSlotItem` / `removeSlotItem`; хуки `useAddSlotItem` / `useMoveSlotItem` / `useRemoveSlotItem`.
- `PlannerPage`: в слоте до 3 карточек-блюд, у каждой ручка «⠿» (drag) и «×» (убрать); название — ссылка на рецепт. Кнопка «+ блюдо» показывается при <3. DnD: id неймспейснуты (`item-…` / `slot-…`), перенос отдельного блюда; правила «полный/дубль» продублированы на клиенте, чтобы заведомо неуспешный перенос не уходил на сервер.

### Проверка
- Backend: **76 тестов** проходят (обновлены тесты планировщика + добавлены add/cap-3/dup/move/move-в-полный/move-дубль/remove/чужой-приватный).
- Live (`:8888`, alice): слот «Пн-завтрак» = 3 блюда (позиции 0/1/2); 4-е → 400; полный цикл add → move(A→B) → remove на пустой неделе: блюдо при переносе **не теряется**; список покупок корректен.
- `tsc` — 0 ошибок; **20 frontend-тестов** проходят; backend+frontend пересобраны; `seed --reset` на новой модели — «24 dishes placed».

---

## 2026-06-19 — Сердечко в списке рецептов: рабочее добавление/удаление из избранного

### Запрос заказчика
В списке рецептов клик по сердечку должен добавлять/убирать из избранного прямо на карточке.

### Диагноз
Кнопка-сердечко на `RecipeCard` уже была и вызывала `useFavoriteToggle`, но **клик до неё не доходил**: заголовок карточки использует `stretched-link`, чей оверлей (`.stretched-link::after`, `z-index: 1`) перекрывает всю карточку. Кнопка была `position-absolute`, но с `z-index: auto` → оверлей поверх неё, клик уходил на ссылку → переход на страницу рецепта вместо переключения избранного.

### Что сделано (frontend)
- Кнопке-сердечку задан `z-index: 2` (поверх оверлея `stretched-link`) — документированный для Bootstrap способ оставить элемент кликабельным поверх stretched-link.
- В обработчик добавлен `e.stopPropagation()` (вдобавок к `preventDefault`).
- Сердечко сделано заметнее и удобнее для нажатия: круглый полупрозрачный фон 34×34 с тенью (раньше — голый символ поверх фото).

### Проверка
- Live (`:8888`, alice): `POST /recipes/{id}/favorite/` дважды → `is_favorited` True → False (добавление/удаление). После переключения список инвалидируется (`useFavoriteToggle`) и сердечко обновляется.
- `tsc` — 0 ошибок; **20 frontend-тестов** проходят; фронтенд пересобран, `:8888` отдаёт 200.
- Примечание: сам баг — слоистый CSS (`z-index`), который jsdom не воспроизводит, поэтому юнит-тест его не поймал бы; проверено вживую.

---

## 2026-06-20 — Сортировка по среднему рейтингу: неоценённые рецепты всплывали наверх

### Симптом (по запросу заказчика)
Сортировка по рейтингу работает неверно.

### Диагноз
`avg_rating` — аннотация `Avg('ratings__value')`, у рецептов без оценок она `NULL`. PostgreSQL по умолчанию сортирует `NULLS FIRST` при `DESC`, поэтому при `ordering=-avg_rating` **неоценённые рецепты оказывались в начале** списка вместо самых высокооценённых. (При `ASC` NULL по умолчанию уходят в конец — там проблема не была видна.) DRF `OrderingFilter` просто делает `order_by('-avg_rating')` и не управляет позицией NULL.

### Что сделано (backend)
- Добавлен `NullsLastOrderingFilter` (подкласс DRF `OrderingFilter`): каждый ключ сортировки переводится в `F(field).desc(nulls_last=True)` / `.asc(nulls_last=True)`, плюс стабильный tiebreak `F('id').desc()` — детерминированная пагинация при равных средних. Применён в `RecipeViewSet`.
- Аннотация `avg_rating` не менялась (остаётся `NULL` для корректного «Нет оценок» на фронте) — чиним только порядок, а не данные.

### Проверка
- Backend: новый тест `test_ordering_by_avg_rating_keeps_unrated_last` (DESC: лучший первый, неоценённый последний; ASC: худший первый, неоценённый последний). Всего **77 тестов** проходят.
- Live (`:8888`): `ordering=-avg_rating` — первые результаты теперь с рейтингом 5.0, неоценённые внизу (раньше первые 8 были без оценок).

---

## 2026-06-20 — Приватный рецепт Алисы виден Бобу: утечка через кэш React Query

### Симптом (по запросу заказчика)
Приватный рецепт Алисы «Хумус домашний» отображается и у Боба.

### Диагноз — баг на фронте, не на бэке
Бэкенд герметичен (проверено вживую под Бобом): хумуса (id 85, `is_public=False`, автор alice) нет в списке (Боб видит 28 = 27 публичных + 1 свой приватный), детальная — 404, в дашборде/избранном — нет.
Утечка — на клиенте: ключи запросов React Query **не привязаны к пользователю** (`['recipes', filters]`, `['recipe', id]`, план, дашборд и т.д.), а `login`/`logout` в `AuthContext` **не очищали кэш**. После выхода Алисы и входа Бобом (или прямого переключения аккаунта) React Query отдавал закэшированный под Алисой список — с её приватным хумусом. Глобальный `staleTime: 60_000` делал утечку стойкой (до минуты без рефетча), а детальная `['recipe', 85]` открывалась прямо из кэша.

### Что сделано (frontend)
- В `AuthContext` добавлен `useQueryClient()`; `queryClient.clear()` вызывается **и при `logout`, и при `login`** — кэш предыдущего пользователя сбрасывается, новый сеанс грузит данные заново под своим токеном. (`QueryClientProvider` обёрнут снаружи `AuthProvider`, поэтому хук доступен.)

### Проверка
- Бэкенд (live, Боб): id 85 в списке — нет; в избранном — нет; детальная — 404; дашборд — без хумуса. Приватность на сервере соблюдается.
- Новый тест `AuthContext.test.tsx`: после `logout` `queryClient` больше не содержит закэшированные `['recipes', …]` и `['recipe', 85]`. Всего **21 frontend-тест** проходит.
- `tsc` — 0 ошибок; фронтенд пересобран, `:8888` отдаёт 200.

### Принятое решение
- Чиним на клиенте сбросом кэша при смене аккаунта (минимально и полно покрывает все per-user запросы). Альтернатива — добавлять id пользователя в каждый queryKey — более инвазивна и легко забыть для нового запроса; `clear()` на границе сессии надёжнее.

---

## 2026-06-20 — Запуск из чистого клона через `docker compose up`

### Запрос заказчика
Должно работать: `git clone <repo> && cd <repo>` → `docker compose up` (без флагов).

### Что мешало
1. README предписывал `cp .env.example .env` и `docker compose up --build -d` — лишние шаги.
2. **Главный риск — переводы строк.** `.gitattributes` не было, `core.autocrlf=true`. На свежем клоне под Windows `entrypoint.sh` (в репо хранится LF) checkout'ился бы как CRLF → шебанг `#!/bin/sh\r` → контейнер не стартует. На этой машине рабочая копия случайно осталась LF, поэтому баг не проявлялся локально, но «выстрелил» бы у грейдера/коллеги.

(`docker-compose.yml` уже был готов: у backend/frontend только `build:` без `image:` — `docker compose up` сам собирает отсутствующие образы; все env-переменные имеют дефолты, `.env` не обязателен.)

### Что сделано
- **`.gitattributes`**: `*.sh`, `entrypoint.sh`, `Dockerfile`, `*.conf` → `text eol=lf`. Гарантирует LF при checkout на любой ОС.
- **backend/Dockerfile**: защитный `RUN sed -i 's/\r$//' entrypoint.sh && chmod +x entrypoint.sh` — образ собирается рабочим даже при CRLF-клоне (belt-and-suspenders к `.gitattributes`).
- **README**: quick-start сведён к `git clone … && cd cookbook` + `docker compose up`; `-d` и `cp .env.example .env` отмечены как опциональные.

### Проверка (буквально команда заказчика, с нуля)
- `docker compose down -v --rmi local` (снёс тома и собранные образы) → `docker compose up -d` (БЕЗ `--build`): Compose сам собрал backend+frontend, db прошёл healthcheck, entrypoint выполнил migrate+seed.
- Результат: API отдаёт **28 рецептов** (seed отработал), фронтенд `:8888` → 200, логин `alice` работает.

---

## 2026-06-20 — Swagger: эндпоинты планировщика без параметров/тел запроса

### Симптом (по запросу заказчика)
В Swagger при вызове `/api/menu-plan/` приходит `{"error": "Параметр week_start обязателен (YYYY-MM-DD)."}`.

### Диагноз
Вью планировщика — обычные `APIView`, читающие `week_start` через `request.query_params.get(...)` и тело через `request.data.get(...)`. drf-spectacular не выводит такие параметры автоматически, поэтому в Swagger не было поля для `week_start` — «Try it out» слал запрос без него → 400. Аналогично у POST/PATCH блюд не было схемы тела запроса.

### Что сделано (backend, аннотации схемы — поведение не меняется)
- Общий `OpenApiParameter` **`week_start`** (query, `format: date`, required, пример) добавлен через `@extend_schema(parameters=[...])` к `MenuPlanView.get` и `ShoppingListView.get`.
- `MealSlotItemCreateView.post` — `@extend_schema(request=MealSlotItemWriteSerializer, responses=MealSlotSerializer)`.
- `MealSlotItemView.patch` — `request=inline_serializer({'slot': IntegerField})`; `delete` — `responses={204: None}`.
- `ShoppingListView.get` — описан ответ (`inline_serializer`, many).

### Проверка
- `manage.py spectacular --validate` генерирует схему без новых ошибок (прежние warning'и в recipes/social не тронуты).
- Live через nginx (`:8888/api/schema/`): `week_start` присутствует (query, required, format date), есть тела `menu-plan/items`. Swagger UI (`/api/docs/`) → 200; теперь у GET-эндпоинтов есть поле `week_start`.
- Все **77 backend-тестов** проходят (декораторы аддитивны).
- Замечание по верификации: `/api/schema/` отдаётся с content-type `application/vnd.oai.openapi`, поэтому PowerShell `Invoke-WebRequest.Content` приходит как массив байт — для подсчёта надо декодировать UTF-8 (иначе ложные нули).

---

## 2026-06-20 — Swagger UI по корневым адресам `/docs`, `/redoc`, `/api-docs`

### Запрос заказчика
Преподаватель открывает Swagger UI по `http://localhost:<port>/docs` (или `/redoc`, `/api-docs`). Сейчас документация только под `/api/docs/`, `/api/redoc/` — корневые пути уходили в SPA-fallback nginx.

### Что сделано
- **backend (`config/urls.py`)**: добавлены корневые маршруты `docs/` и `api-docs/` → `SpectacularSwaggerView`, `redoc/` → `SpectacularRedocView`. Все используют существующую схему (`url_name="schema"` → `/api/schema/`). Старые `/api/docs/`, `/api/redoc/` сохранены (обратная совместимость).
- **nginx (`frontend/nginx.conf`)**: regex-location `^/(docs|redoc|api-docs)(/|$)` проксирует эти пути на backend (одним блоком, до SPA-fallback). Не конфликтует с `/api/` (другой префикс).

### Проверка
- Live через nginx (`:8888`): `/docs`, `/docs/`, `/redoc`, `/redoc/`, `/api-docs`, `/api-docs/` → **200** (Swagger/ReDoc). Без слеша работает через 301 от Django `APPEND_SLASH`.
- Страница `/docs/` ссылается на `/api/schema/` (200) — спецификация подгружается; «Try it out» бьёт в `/api/...` через nginx.
- Старые `/api/docs/`, `/api/redoc/` — по-прежнему 200.
- Новый `test_docs.py` (6 путей → 200). Всего **83 backend-теста**.
- README: таблица адресов обновлена (`/docs`, `/redoc`, `/api-docs`, `/api/schema/`).

---

## 2026-06-20 — Загрузка фото при создании/редактировании рецепта

### Запрос заказчика
Возможность загрузить фото при создании рецепта.

### Контекст
Бэкенд уже принимал `photo` (`RecipeWriteSerializer.Meta.fields`, модель — `ImageField(upload_to='recipes/')`), DRF-парсеры по умолчанию включают `MultiPartParser`. Не хватало только UI на фронте — форма отправляла JSON, без поля файла.

### Решение
Создание рецепта содержит вложенные поля (ingredients/steps — массивы объектов), которые неудобно слать через multipart. Поэтому: рецепт создаётся/обновляется как раньше (JSON), а фото догружается **отдельным multipart-PATCH** на `/recipes/{id}/` (partial update — остальные поля не трогаются).

### Что сделано (frontend; бэкенд не менялся)
- `recipesApi.uploadPhoto(id, file)` — `PATCH` с `FormData`; `Content-Type: undefined`, чтобы axios/браузер сам проставил `multipart/form-data` с boundary (дефолт клиента — JSON).
- Хук `useUploadRecipePhoto` (инвалидация `recipes` + `recipe/{id}`).
- `RecipeFormPage`: поле `<input type="file" accept="image/*">` в карточке «Фото», превью выбранного файла (`URL.createObjectURL` + revoke) и текущего фото при редактировании; клиентская валидация (только изображения, ≤ 5 МБ). После create/update, если выбран файл, — `uploadPhoto`.

### Проверка
- Backend-тесты: `test_author_can_upload_photo` (multipart PATCH → 200, `photo` сохранён в `recipes/`), `test_upload_rejects_non_image` (текстовый файл → 400). Всего **85 тестов**.
- Live (`:8888`): media отдаётся через nginx (`/media/recipes/...` → 200, image/jpeg). Загруженные файлы попадают в общий том `media_files` и отдаются так же.
- `tsc` — 0 ошибок; **21 frontend-тест**; фронтенд пересобран.
